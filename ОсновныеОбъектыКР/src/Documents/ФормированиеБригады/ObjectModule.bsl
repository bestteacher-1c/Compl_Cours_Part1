...
Процедура ПроверитьЗаполнениеРеквизитов(Отказ)
	...
	НеУказанВодитель = ЗначениеЗаполнено(Водитель);
	НеУказанПервыйГрузчик = ЗначениеЗаполнено(ПервыйГрузчик);
	НеУказанВторойГрузчик = ЗначениеЗаполнено(ВторойГрузчик);
	НеУказанМашина = ЗначениеЗаполнено(Машина);

	Если НеУказанВодитель ИЛИ НеУказанПервыйГрузчик ИЛИ НеУказанВторойГрузчик ИЛИ
		 НеУказанМашина ИЛИ НачальноеЗначениеСчетчика <= 0 ИЛИ
